...
public class BallController : MonoBehaviour {
	...
	void Update () {
		if (inControl) {
			...
			if ((Input.GetAxisRaw("Vertical") != 0) || (Input.GetAxisRaw("Horizontal") != 0) )
				{
					shootAngle = Mathf.Atan2 (Input.GetAxis ("Vertical"),Input.GetAxis ("Horizontal")) * Mathf.Rad2Deg;
	...
	void Shoot()
	{
		...
		rb.AddForce(new Vector2(Input.GetAxisRaw ("Horizontal"),Input.GetAxisRaw ("Vertical")) * 25f, ForceMode2D.Impulse);
		canSpawn = true;
	}
}
